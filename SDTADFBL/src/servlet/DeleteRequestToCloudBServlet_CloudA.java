...
@WebServlet("/DeleteRequestToCloudBServlet_CloudA")
public class DeleteRequestToCloudBServlet_CloudA extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(fid!=0) 
		{
			...
			while(rs.next()) 
			{
				...
				i = new SecurityDAO().cloudADeleteRequestToCloudB(b);
			}
			if(i!=0) 
			{
				j = new SecurityDAO().cloudADeleteFileToCloudB(fid,uid);
			}
			if(j!=0) {
				
				t="DeleteRequestFromDo_CloudA.jsp?status=Delete Request Send to Cloud B";
			}else 
			{
				t="DeleteRequestFromDo_CloudA.jsp?status=Not Send";
			}
			}catch (Exception e) {
				e.printStackTrace();
				t="DeleteRequestFromDo_CloudA.jsp?status=Check Console Error";
			}
		}else 
		{
